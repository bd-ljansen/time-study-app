...
class TimeStudyApp(QMainWindow):
    def __init__(self):
        ...
        self.cap2 = None
        self.active_video_path2 = ""
        self.vid2_offset_ms = 0
        self.videos_linked = True
    ...
    def next_frame(self):
        if self.is_playing:
            ...
            # Update Primary Video View (Only if Linked)
            if self.videos_linked and self.cap and self.cap.isOpened():
            ...
            # Update Secondary Video View
            if self.dual_camera_mode and self.cap2 and self.cap2.isOpened():
                for _ in range(frames_to_skip):
                    self.cap2.grab()
                ...
                if ret2:
                    ...
                    q_img2 = QImage(frame2.data, w2, h2, ch2 * w2, QImage.Format_RGB888)
                    self.video_view2.set_frame(QPixmap.fromImage(q_img2))
                    
                    # If Unlinked, the UI should track Video 2's progress instead
                    if not self.videos_linked:
                        current_ms2 = int(self.cap2.get(cv2.CAP_PROP_POS_MSEC))
                        dur2 = int((self.cap2.get(cv2.CAP_PROP_FRAME_COUNT) / (self.cap2.get(cv2.CAP_PROP_FPS) or 30.0)) * 1000)
                        self.slider.blockSignals(True)
                        self.slider.setValue(current_ms2)
                        self.slider.blockSignals(False)
                        self.time_label.setText(f"{self.format_ms(current_ms2)} / {self.format_ms(dur2)} (Scrubbing Video 2)")
                else:
                    if not self.videos_linked:
                        self.toggle_play()
    ...
    def toggle_play(self):
        ...
        if (self.cap and self.cap.isOpened()) or (self.cap2 and self.cap2.isOpened()):
    ...
    def toggle_dual_camera_mode(self, checked):
        """Toggles the dual camera view."""
        self.dual_camera_mode = checked
        self.video_view2.setVisible(checked)
        self.open_vid2_btn.setVisible(checked)
        self.link_btn.setVisible(checked)
        
        if not checked and not self.videos_linked:
            self.link_btn.setChecked(True) # Force relink if dual mode is turned off
            
        if checked and not self.cap2:
            self.open_file_dialog2()

    def on_link_toggled(self, checked):
        """Handles switching between synced playback and Video-2-only control."""
        self.videos_linked = checked
        if checked:
            self.link_btn.setText("🔗 Linked")
            self.link_btn.setStyleSheet("font-weight: bold; background-color: #22c55e; color: white; padding: 4px 12px;")
            
            # Calculate the new offset based on where the user left Video 2
            pos1 = self.cap.get(cv2.CAP_PROP_POS_MSEC) if (self.cap and self.cap.isOpened()) else 0
            pos2 = self.cap2.get(cv2.CAP_PROP_POS_MSEC) if (self.cap2 and self.cap2.isOpened()) else 0
            self.vid2_offset_ms = int(pos2 - pos1)
            
            # Restore slider mapping to Video 1
            if self.cap and self.cap.isOpened():
                self.slider.blockSignals(True)
                self.slider.setRange(0, self.duration_ms)
                self.slider.setValue(int(pos1))
                self.slider.blockSignals(False)
                self.time_label.setText(f"{self.format_ms(pos1)} / {self.format_ms(self.duration_ms)} (◄ / ► Arrow Keys = ±1s)")
        else:
            self.link_btn.setText("🔓 Unlinked (Syncing)")
            self.link_btn.setStyleSheet("font-weight: bold; background-color: #eab308; color: white; padding: 4px 12px;")
            
            # Switch slider mapping to Video 2 so you can scrub it independently
            if self.cap2 and self.cap2.isOpened():
                pos2 = self.cap2.get(cv2.CAP_PROP_POS_MSEC)
                dur2 = int((self.cap2.get(cv2.CAP_PROP_FRAME_COUNT) / (self.cap2.get(cv2.CAP_PROP_FPS) or 30.0)) * 1000)
                self.slider.blockSignals(True)
                self.slider.setRange(0, dur2)
                self.slider.setValue(int(pos2))
                self.slider.blockSignals(False)
                self.time_label.setText(f"{self.format_ms(pos2)} / {self.format_ms(dur2)} (Scrubbing Video 2)")

    def open_file_dialog(self):
        """Triggers the primary video loading dialog."""
        file_path, _ = QFileDialog.getOpenFileName(self, "Open Video File 1", "", "Video Files (*.mp4 *.avi *.mkv *.mov *.mts *.MTS)")
        if file_path:
            self.add_video_group(file_path)

    def open_file_dialog2(self):
        """Triggers the secondary video loading dialog for Dual Camera Mode."""
        file_path, _ = QFileDialog.getOpenFileName(self, "Open Video File 2 (Dual Camera)", "", "Video Files (*.mp4 *.avi *.mkv *.mov *.mts *.MTS)")
        if file_path:
            self.load_video2(file_path)

    def load_video2(self, file_path):
        """Loads and initializes the second camera feed."""
        if not file_path or not os.path.exists(file_path):
            return
        if self.cap2:
            self.cap2.release()
        
        self.active_video_path2 = file_path
        self.cap2 = cv2.VideoCapture(file_path)
        
        # Apply the current position and offset directly
        current_ms = self.slider.value() if (self.cap and self.cap.isOpened()) else 0
        target_ms2 = max(0, current_ms + self.vid2_offset_ms)
        self.cap2.set(cv2.CAP_PROP_POS_MSEC, target_ms2)
        ...
        if ret:
            ...
            h, w, ch = frame.shape
            q_img = QImage(frame.data, w, h, ch * w, QImage.Format_RGB888)
            self.video_view2.set_frame(QPixmap.fromImage(q_img))
        
        self.video_view2.reset_zoom()

    def seek_position(self, ms):
        """Jumps to a specific timestamp."""
        if self.videos_linked:
            # Linked: Move Video 1 to target, Video 2 to target + offset
            if self.cap and self.cap.isOpened():
            ...
            if self.dual_camera_mode and self.cap2 and self.cap2.isOpened():
                target_ms2 = max(0, ms + self.vid2_offset_ms)
                self.cap2.set(cv2.CAP_PROP_POS_MSEC, target_ms2)
                ret2, frame2 = self.cap2.read()
                if ret2:
                    frame2 = cv2.cvtColor(frame2, cv2.COLOR_BGR2RGB)
            ...
            # Unlinked: Move ONLY Video 2 (Video 1 is frozen)
            if self.dual_camera_mode and self.cap2 and self.cap2.isOpened():
                self.cap2.set(cv2.CAP_PROP_POS_MSEC, ms)
                ret2, frame2 = self.cap2.read()
                if ret2:
                    frame2 = cv2.cvtColor(frame2, cv2.COLOR_BGR2RGB)
                    h2, w2, ch2 = frame2.shape
                    ...
                    self.slider.setValue(int(ms))
                    self.slider.blockSignals(False)
                    self.time_label.setText(f"{self.format_ms(ms)} / {self.format_ms(dur2)} (Scrubbing Video 2)")

    def step_time(self, delta_ms):
        """Nudges video(s) forward or backward."""
        if not self.videos_linked:
            # Step only Video 2
            if self.cap2 and self.cap2.isOpened():
                base_ms = self.target_seek_ms if self.seek_timer.isActive() else self.cap2.get(cv2.CAP_PROP_POS_MSEC)
                dur2 = int((self.cap2.get(cv2.CAP_PROP_FRAME_COUNT) / (self.cap2.get(cv2.CAP_PROP_FPS) or 30.0)) * 1000)
                self.target_seek_ms = max(0, min(dur2, base_ms + delta_ms))
                self.slider.blockSignals(True)
                self.slider.setValue(int(self.target_seek_ms))
                self.slider.blockSignals(False)
                self.time_label.setText(f"{self.format_ms(self.target_seek_ms)} / {self.format_ms(dur2)} (Scrubbing Video 2...)")
                self.seek_timer.start(10)
        else:
            # Step both videos (based on Video 1 time)
            if self.cap and self.cap.isOpened():
    ...
    def reset_both_zooms(self):
        """Resets zoom on both video feeds."""
    ...
    def create_menu_bar(self):
        ...
        self.dual_cam_action = QAction("Dual Camera Mode", self)
        self.dual_cam_action.setCheckable(True)
        self.dual_cam_action.triggered.connect(self.toggle_dual_camera_mode)
        file_menu.addAction(self.dual_cam_action)
    ...
    def deferred_seek(self, ms):
        """Debounces slider scrubbing to prevent OpenCV from locking the UI."""
        self.target_seek_ms = ms
        
        # Instantly update the UI text so it feels responsive
        if self.videos_linked:
            self.time_label.setText(f"{self.format_ms(ms)} / {self.format_ms(self.duration_ms)} (Scrubbing...)")
        elif self.cap2 and self.cap2.isOpened():
            dur2 = int((self.cap2.get(cv2.CAP_PROP_FRAME_COUNT) / (self.cap2.get(cv2.CAP_PROP_FPS) or 30.0)) * 1000)
            self.time_label.setText(f"{self.format_ms(ms)} / {self.format_ms(dur2)} (Scrubbing Video 2...)")
            
        # Wait 150 milliseconds before triggering the heavy video render
    ...
    def init_ui(self):
        ...
        # Dual Video Stack Layout
        self.video_stack_layout = QVBoxLayout()
        self.video_stack_layout.setSpacing(6)

        # Primary Video
        self.video_view = ZoomableVideoView()
        self.video_view.setStyleSheet("background-color: #1e1e1e; border: 1px solid #333;")
        self.video_stack_layout.addWidget(self.video_view, stretch=1)

        # Secondary Video (Dual Camera Mode)
        self.video_view2 = ZoomableVideoView()
        self.video_view2.setStyleSheet("background-color: #1e1e1e; border: 1px solid #333;")
        self.video_view2.setVisible(False)
        self.video_stack_layout.addWidget(self.video_view2, stretch=1)

        left_layout.addLayout(self.video_stack_layout, stretch=5)
        ...
        self.slider.sliderMoved.connect(self.deferred_seek)
        ...
        self.open_vid_btn = QPushButton("+ Load Video 1")
        ...
        self.open_vid2_btn = QPushButton("+ Load Video 2")
        self.open_vid2_btn.setFocusPolicy(Qt.NoFocus)
        self.open_vid2_btn.setStyleSheet("font-weight: bold; background-color: #0284c7; color: white;")
        self.open_vid2_btn.setVisible(False)
        self.open_vid2_btn.clicked.connect(self.open_file_dialog2)
        controls_layout.addWidget(self.open_vid2_btn)
        ...
        controls_layout.addWidget(self.link_btn)
        ...
        self.reset_zoom_btn.clicked.connect(self.reset_both_zooms)
    ...
    def toggle_view_mode(self, index):
        if index == 1:
            ...
            self.open_vid2_btn.setEnabled(False)
            ...
            self.open_vid_btn.setStyleSheet("font-weight: bold;")
            self.open_vid2_btn.setEnabled(True)
    ...
    def build_category_view(self):
        ...
        for cat_name, data in sorted_cats:
            ...
            cat_item.setText(3, self.format_ms(data["total_time"]))
            ...
            for seg in sorted_segs:
                ...
                self.category_tree.setItemWidget(child, 3, action_widget)
    ...
    def export_category_videos(self):
        ...
        out_dir = QFileDialog.getExistingDirectory(self, "Select Output Directory for MP4")
    ...
    def replace_video_for_group(self, group_item):
        ...
        new_path, _ = QFileDialog.getOpenFileName(self, f"Select Replacement Video File for {grp_name}", "", "Video Files (*.mp4 *.avi *.mkv *.mov *.mts *.MTS)")
    ...
    def switch_active_video(self, group_item, target_ms=0):
        ...
        if not file_path or not os.path.exists(file_path):
            new_path, _ = QFileDialog.getOpenFileName(self, f"Locate Missing Video ({os.path.basename(str(file_path))})", "", "Video Files (*.mp4 *.avi *.mkv *.mov *.mts *.MTS)")
    ...
    def _write_project_file(self, path):
        ...
        project_data = {
            ...
            "active_video_path2": self.active_video_path2,
            "dual_camera_mode": self.dual_camera_mode,
            "vid2_offset_ms": self.vid2_offset_ms,
            "last_position_ms": current_ms,
            "video_groups": groups_data, 
            "custom_gen_cats": list(self.custom_gen_cats), 
            "custom_spec_cats": list(self.custom_spec_cats)
        }
        try:
    ...
    def open_project_dialog(self):
        ...
        active_path2 = data.get("active_video_path2", "")
        dual_mode = data.get("dual_camera_mode", False)
        
        self.vid2_offset_ms = data.get("vid2_offset_ms", 0)
        
        if dual_mode:
            self.dual_cam_action.setChecked(True)
            self.toggle_dual_camera_mode(True)
            if active_path2:
                self.load_video2(active_path2)
