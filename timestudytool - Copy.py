...
from PyQt5.QtCore import Qt, QTimer, QEvent, QObject, QRect, QSettings
...
class TimeStudyApp(QMainWindow):
    def __init__(self):
        ...
        self.project_path = ""
        self.settings = QSettings("Luke Jansen", "Time Study Tool")
    ...
    def create_menu_bar(self):
        ...
        export_full_action = QAction("Export Full Project Video...", self)
        export_full_action.triggered.connect(self.export_full_project)
        file_menu.addAction(export_full_action)
    ...
    def get_category_segments(self, groups_data):
        ...
        for group_index, group in enumerate(groups_data):
            ...
            for row_index, row in enumerate(rows):
                ...
                # The last action continues until the first timestamp in a later video.
        ...
        for i, (cat, segments) in enumerate(cat_segments.items()):
            ...
            for j, (vid_path, start_ms, end_ms) in enumerate(segments):
                ...
                while True:
                    ...
                    # Decode and write only the target frame
                    ret, frame = cap.read()
                    if not ret: break
                    ...
                    # FAST FORWARD: Grab skipped frames without decoding them
                    if speed_factor > 1:
    ...
    def export_full_project(self):
        """Merges all videos in the project end-to-end at a chosen speed multiplier."""
        base_state = self.saved_video_state if self.view_mode == "category" else self.get_current_state()
        if not base_state:
            QMessageBox.information(self, "Export", "There is no video data loaded to export.")
            return
        ...
        for g_data in base_state:
            vid_path = g_data.get("video_path", "")
            if vid_path and os.path.exists(vid_path):
                valid_videos.append(vid_path)

        if not valid_videos:
            QMessageBox.information(self, "Export", "No valid video files found in the project.")
            return

        speed_str, ok = QInputDialog.getItem(self, "Export Speed", "Select export speed multiplier:", ["1x", "2x", "4x", "8x", "16x"], 0, False)
        if not ok: return
        speed_factor = int(speed_str.replace("x", ""))

        out_path, _ = QFileDialog.getSaveFileName(self, "Save Full Project Video", "Project_Export.mp4", "Video Files (*.mp4)")
        if not out_path: return

        progress = QProgressDialog("Exporting Full Project...", "Cancel", 0, len(valid_videos), self)
        progress.setWindowTitle("Exporting Video")
        progress.setWindowModality(Qt.WindowModal)
        progress.show()
        ...
        for i, vid_path in enumerate(valid_videos):
            if progress.wasCanceled(): break
            progress.setLabelText(f"Exporting Video {i+1} of {len(valid_videos)}...")
            progress.setValue(i)
            QApplication.processEvents()
        ...
        progress.setValue(len(valid_videos))
        if not progress.wasCanceled():
    ...
    def _write_project_file(self, path):
        ...
        self.settings.setValue("last_project_path", os.path.abspath(path))
